...
function comparePluginRows(first, second) {
  let order = compareCodeUnits(first.name.toLowerCase(), second.name.toLowerCase())
  if (order !== 0) return order
  order = compareCodeUnits(first.name, second.name)
  if (order !== 0) return order
  return compareCodeUnits(first.url, second.url)
}
